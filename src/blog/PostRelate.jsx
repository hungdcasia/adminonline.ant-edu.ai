...
const PostRelate = ({ post }) => {
    let page = 1
    const { data: postsResult } = useQuery(["posts-relate", post.id], () => getPosts());
    ...
    const getPosts = () => {
        ...
        let filterOptions = [
            ...
            { field: 'categoryId', operator: '==', value: post.categoryId },
            { field: 'id', operator: '!=', value: post.id },
        ]
        let sortOptions = { sort: 'views', direction: 'desc' }
        return postService.filter(pagingOptions, filterOptions, sortOptions)
    }

    let postChunks = ArrayHelpers.arrayToChunks(posts, 3)

    if (posts.length == 0)
        return (<></>)

    return (
        <div className='post-interest w-100 px-2 rounded-lg' >
            <div className='w-100 py-3'>
                <h4><FontAwesomeIcon className='text-orange' icon={allIcon.faComments} /> Bài viết liên quan</h4>
            </div>
            <div className='w-100'>
                <Carousel
                    showArrows={true}
                    ...
                    {postChunks.map((chunk, index) => (
                        <div key={index} className='w-100 row'>
                            {chunk.map(post => (
                                <div key={post.id} className='col-4 px-2 py-1' style={{ minHeight: '90px' }}>
